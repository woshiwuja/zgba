...
pub export fn pt_in_rect(arg_pt: [*c]const POINT, arg_rc: [*c]const struct_RECT) c_int {
    var pt = arg_pt;
    _ = &pt;
    var rc = arg_rc;
    _ = &rc;
    if ((pt.*.x < rc.*.left) or (pt.*.x >= rc.*.right)) return 0;
    if ((pt.*.y < rc.*.top) or (pt.*.y >= rc.*.bottom)) return 0;
    return 1;
}
...
pub export fn rc_normalize(arg_rc: [*c]RECT) [*c]RECT {
    var rc = arg_rc;
    _ = &rc;
    var tmp: c_int = undefined;
    _ = &tmp;
    if (rc.*.left > rc.*.right) while (true) {
        tmp = rc.*.left;
        rc.*.left = rc.*.right;
        rc.*.right = tmp;
        if (!false) break;
    };
    if (rc.*.top > rc.*.bottom) while (true) {
        tmp = rc.*.top;
        rc.*.top = rc.*.bottom;
        rc.*.bottom = tmp;
        if (!false) break;
...
pub export fn vec_cross(arg_vd: [*c]VECTOR, arg_va: [*c]const VECTOR, arg_vb: [*c]const VECTOR) [*c]VECTOR {
    var vd = arg_vd;
    _ = &vd;
    var va = arg_va;
    _ = &va;
