...
pub export fn srf_init(arg_srf: [*c]TSurface, arg_type: enum_ESurfaceType, arg_data: ?*const anyopaque, arg_width: uint, arg_height: uint, arg_bpp: uint, arg_pal: [*c]@"u16") void {
    var srf = arg_srf;
    _ = &srf;
    var @"type" = arg_type;
    _ = &@"type";
    ...
    var pal = arg_pal;
    _ = &pal;
    _ = memset(@as(?*anyopaque, @ptrCast(srf)), @as(c_int, 0), @sizeOf(TSurface));
    srf.*.data = @as([*c]@"u8", @ptrCast(@volatileCast(@constCast(data))));
    srf.*.type = @as(@"u8", @bitCast(@as(u8, @truncate(@"type"))));
    while (true) {
        switch (@"type" & @as(c_uint, @bitCast(~SRF_ALLOCATED))) {
            @as(c_uint, @bitCast(@as(c_int, 4))) => {
                bpp = 4;
                srf.*.pitch = srf_align(width, @as(uint, @bitCast(@as(c_int, 4)))) *% @as(uint, @bitCast(@as(c_int, 8)));
                break;
            },
            @as(c_uint, @bitCast(@as(c_int, 5))) => {
                bpp = 4;
                srf.*.pitch = srf_align(height, @as(uint, @bitCast(@as(c_int, 4)))) *% @as(uint, @bitCast(@as(c_int, 8)));
                break;
            },
            @as(c_uint, @bitCast(@as(c_int, 6))) => {
                bpp = 8;
                srf.*.pitch = srf_align(width, @as(uint, @bitCast(@as(c_int, 8)))) *% @as(uint, @bitCast(@as(c_int, 8)));
                break;
            },
    ...
    srf.*.width = @as(@"u16", @bitCast(@as(c_ushort, @truncate(width))));
    srf.*.height = @as(@"u16", @bitCast(@as(c_ushort, @truncate(height))));
    srf.*.bpp = @as(@"u8", @bitCast(@as(u8, @truncate(bpp))));
    if (pal != @as([*c]@"u16", @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        srf.*.palSize = @as(@"u16", @bitCast(@as(c_short, @truncate(@as(c_int, 1) << @intCast(bpp)))));
        srf.*.palData = pal;
    }
}
pub export fn srf_pal_copy(arg_dst: [*c]const TSurface, arg_src: [*c]const TSurface, arg_count: uint) void {
    var dst = arg_dst;
    _ = &dst;
    var src = arg_src;
    _ = &src;
    var count = arg_count;
    _ = &count;
    if (((dst != null) and (src != null)) and (count != 0)) {
        memcpy16(@as(?*anyopaque, @ptrCast(dst.*.palData)), @as(?*const anyopaque, @ptrCast(src.*.palData)), count);
    }
}
pub export fn srf_get_ptr(arg_srf: [*c]const TSurface, arg_x: uint, arg_y: uint) ?*anyopaque {
    var srf = arg_srf;
    _ = &srf;
    var x = arg_x;
    _ = &x;
    var y = arg_y;
    _ = &y;
    var bpp: uint = @as(uint, @bitCast(@as(c_uint, srf.*.bpp)));
    _ = &bpp;
    var pitch: uint = srf.*.pitch;
    _ = &pitch;
    while (true) {
        switch (@as(c_int, @bitCast(@as(c_uint, srf.*.type))) & ~SRF_ALLOCATED) {
            @as(c_int, 4), @as(c_int, 6) => return @as(?*anyopaque, @ptrCast(&srf.*.data[((((y / @as(uint, @bitCast(@as(c_int, 8)))) *% pitch) +% ((y % @as(uint, @bitCast(@as(c_int, 8)))) *% bpp)) +% (((x / @as(uint, @bitCast(@as(c_int, 8)))) *% @as(uint, @bitCast(@as(c_int, 8)))) *% bpp)) +% (((x % @as(uint, @bitCast(@as(c_int, 8)))) *% bpp) / @as(uint, @bitCast(@as(c_int, 8))))])),
            @as(c_int, 5) => return @as(?*anyopaque, @ptrCast(&srf.*.data[((y *% @as(uint, @bitCast(@as(c_int, 4)))) +% ((x / @as(uint, @bitCast(@as(c_int, 8)))) *% pitch)) +% ((x % @as(uint, @bitCast(@as(c_int, 8)))) / @as(uint, @bitCast(@as(c_int, 2))))])),
            else => return @as(?*anyopaque, @ptrCast(&srf.*.data[(y *% pitch) +% ((x *% bpp) / @as(uint, @bitCast(@as(c_int, 8))))])),
        }
        break;
    }
    return null;
...
// C:\Users\Gabusi_F\Desktop\zig0.14\zig-x86_64-windows-0.14.1\lib\libc\include\any-windows-any/string.h:16:9
