...
pub export fn se_window(arg_sbb: [*c]SCR_ENTRY, arg_left: c_int, arg_top: c_int, arg_right: c_int, arg_bottom: c_int, arg_se0: SCR_ENTRY) void {
    var sbb = arg_sbb;
    _ = &sbb;
    var left = arg_left;
    _ = &left;
    ...
    _ = &iy;
    right -= left + @as(c_int, 1);
    bottom -= top + @as(c_int, 1);
    sbb += @as(usize, @bitCast(@as(isize, @intCast((top * @as(c_int, 32)) + left))));
    sbb[@as(c_uint, @intCast(@as(c_int, 0)))] = se0;
    (blk: {
        const tmp = right;
        if (tmp >= 0) break :blk sbb + @as(usize, @intCast(tmp)) else break :blk sbb - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
    }).* = @as(SCR_ENTRY, @bitCast(@as(c_short, @truncate(@as(c_int, @bitCast(@as(c_uint, se0))) + @as(c_int, 2)))));
    (blk: {
        const tmp = bottom * @as(c_int, 32);
        if (tmp >= 0) break :blk sbb + @as(usize, @intCast(tmp)) else break :blk sbb - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
    }).* = @as(SCR_ENTRY, @bitCast(@as(c_short, @truncate(@as(c_int, @bitCast(@as(c_uint, se0))) + @as(c_int, 6)))));
    (blk: {
        const tmp = (bottom * @as(c_int, 32)) + right;
        if (tmp >= 0) break :blk sbb + @as(usize, @intCast(tmp)) else break :blk sbb - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
    }).* = @as(SCR_ENTRY, @bitCast(@as(c_short, @truncate(@as(c_int, @bitCast(@as(c_uint, se0))) + @as(c_int, 8)))));
    {
        ix = 1;
        while (ix < right) : (ix += 1) {
            (blk: {
                const tmp = ix;
                if (tmp >= 0) break :blk sbb + @as(usize, @intCast(tmp)) else break :blk sbb - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
            }).* = @as(SCR_ENTRY, @bitCast(@as(c_short, @truncate(@as(c_int, @bitCast(@as(c_uint, se0))) + @as(c_int, 1)))));
            (blk: {
                const tmp = (bottom * @as(c_int, 32)) + ix;
                if (tmp >= 0) break :blk sbb + @as(usize, @intCast(tmp)) else break :blk sbb - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
            }).* = @as(SCR_ENTRY, @bitCast(@as(c_short, @truncate(@as(c_int, @bitCast(@as(c_uint, se0))) + @as(c_int, 7)))));
        }
    }
    sbb += @as(usize, @bitCast(@as(isize, @intCast(@as(c_int, 32)))));
    {
        iy = 1;
        while (iy < bottom) : (iy += 1) {
            sbb[@as(c_uint, @intCast(@as(c_int, 0)))] = @as(SCR_ENTRY, @bitCast(@as(c_short, @truncate(@as(c_int, @bitCast(@as(c_uint, se0))) + @as(c_int, 3)))));
            {
                ix = 1;
                while (ix < right) : (ix += 1) {
                    (blk: {
                        const tmp = ix;
                        if (tmp >= 0) break :blk sbb + @as(usize, @intCast(tmp)) else break :blk sbb - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
                    }).* = @as(SCR_ENTRY, @bitCast(@as(c_short, @truncate(@as(c_int, @bitCast(@as(c_uint, se0))) + @as(c_int, 4)))));
                }
            }
            (blk: {
                const tmp = right;
                if (tmp >= 0) break :blk sbb + @as(usize, @intCast(tmp)) else break :blk sbb - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
            }).* = @as(SCR_ENTRY, @bitCast(@as(c_short, @truncate(@as(c_int, @bitCast(@as(c_uint, se0))) + @as(c_int, 5)))));
            sbb += @as(usize, @bitCast(@as(isize, @intCast(@as(c_int, 32)))));
        }
    }
}
